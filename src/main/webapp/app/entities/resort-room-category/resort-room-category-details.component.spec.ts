/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import ResortRoomCategoryDetails from './resort-room-category-details.vue';
import ResortRoomCategoryService from './resort-room-category.service';
import AlertService from '@/shared/alert/alert.service';

type ResortRoomCategoryDetailsComponentType = InstanceType<typeof ResortRoomCategoryDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const resortRoomCategorySample = { id: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('ResortRoomCategory Management Detail Component', () => {
    let resortRoomCategoryServiceStub: SinonStubbedInstance<ResortRoomCategoryService>;
    let mountOptions: MountingOptions<ResortRoomCategoryDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      resortRoomCategoryServiceStub = sinon.createStubInstance<ResortRoomCategoryService>(ResortRoomCategoryService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          resortRoomCategoryService: () => resortRoomCategoryServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        resortRoomCategoryServiceStub.find.resolves(resortRoomCategorySample);
        route = {
          params: {
            resortRoomCategoryId: '' + 123,
          },
        };
        const wrapper = shallowMount(ResortRoomCategoryDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.resortRoomCategory).toMatchObject(resortRoomCategorySample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        resortRoomCategoryServiceStub.find.resolves(resortRoomCategorySample);
        const wrapper = shallowMount(ResortRoomCategoryDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
